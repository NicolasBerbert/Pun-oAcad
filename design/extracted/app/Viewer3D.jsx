...
function rebuild(s, data, layers, highlight) {
  ...
  if (layers.forcas) {
    ...
    if (Fsk && Fsk > 0) {
      const len = Math.max(20, Math.min(60, Fsk / 5));
      const arrow = makeForceArrow(0xb42318, new THREE.Vector3(0, colTopY + len + 8, 0), new THREE.Vector3(0, colTopY + 4, 0), len);
      forcesGroup.add(arrow);
      forcesGroup.add(makeTextSprite(`Fsk = ${Fsk} kN`, new THREE.Vector3(0, colTopY + len + 14, 0), 0xb42318));
    }

    if (Mxk && Mxk > 0) {
      // Mx is moment ABOUT x-axis → rotation in YZ plane
      const r = Math.max(15, Math.min(40, Math.log(Mxk + 1) * 5));
      const mom = makeMomentArrow(0x9333ea, 'x', r, colTopY);
      forcesGroup.add(mom);
      forcesGroup.add(makeTextSprite(`Mxk = ${Mxk} kN·cm`, new THREE.Vector3(0, colTopY + 6, r + 10), 0x9333ea));
    }
    if (Myk && Myk > 0) {
      const r = Math.max(15, Math.min(40, Math.log(Myk + 1) * 5));
      const mom = makeMomentArrow(0x16a394, 'z', r, colTopY);
      forcesGroup.add(mom);
...
function makeMomentArrow(color, axis, radius, y) {
